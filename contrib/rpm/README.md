...
The RPM spec file provided here is for BICOScoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://bicoscoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[BICOScoin git repository](https://github.com/bicosgb/bicos) but are not part of
...
Currently only `Source10` is used, for the example `bicoscoincore.conf` file.
...
in the bicoscoin git repository. At present only an SVG version of the BICOScoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the bicoscoin
...
needed when 0.12.1 is released, a proper fix is already in the BICOScoin git
master branch.
...
idea to build BICOScoin Core against a static version of that library compiled
...
The `bicoscoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `bicoscoin-server` package. At this point, most RPM based
...
The `bicoscoin-server` package should have SELinux support. How to properly do
...
BICOScoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/bicoscoin/). He has been
packaging BICOScoin for Fedora at least since 2012.
...
4. I renamed the `bicoscoin` package that contains the Qt GUI to `bicoscoin-core` as
that appears to be how the general population refers to it, in contrast to
`bicoscoin-xt` or `bicoscoin-classic`. I wanted to make sure the general population
