...
###   bicoscoincore.org and bicoscoin.org and compares them.
...
WORKINGDIR="/tmp/bicoscoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bicoscoincore.org"
HOST2="https://bicoscoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="bicoscoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bicoscoin-',
...
if [ $? -ne 0 ]; then
   echo "bicoscoin.org failed to provide signature file, but bicoscoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bicoscoin.org and bicoscoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BICOScoin Core binary release signing key installed?"
   fi
