#!/bin/bash
# Copyright (c) 2013 The BICOScoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete BICOScoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no BICOScoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old BICOScoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected BICOScoin 0.7 datadir."
    ;;
  3)
    echo "Detected BICOScoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected BICOScoin 0.8 datadir."
    ;;
esac
