    ...
    const std::string& strGenesisBlockSignatureBase64)
{
    ...
    genesis.sig.resize(BICOS_SIGN_BYTES * genesis.iVerificationLength);
...
public:
	CMainParams() {
        strNetworkID = "main";
        // bicos : 반감기값, nSubsidyHalvingInterval 값으로 설정된 블록마다 보상이 절반으로 줄어듬
		...
		//consensus.BCSHeight = std::numeric_limits<int>::max(); //491407; // Around 10/25/2017 12:00 UTC
        //consensus.BCSPremineWindow = 8000;
        consensus.BCSZawyLWMAHeight = std::numeric_limits<int>::max(); // Not activated on mainnet
        consensus.BCSEquihashForkHeight = std::numeric_limits<int>::max(); // Not activated on mainnet
        //consensus.BCSPremineEnforceWhitelist = true;
        ...
        // bicos : Digishield난이도 조정 알고리즘의 매개 변수입니다. (30블록마다 난이도가 변경)
        ...
        consensus.BCSMaxFutureBlockTime = 12 * 10 * 60; // 120 mins
        
		//bicos : test, 약 1시간마다 갱신, 블럭은 3초 
		...
		//vSeeds.emplace_back("mainnet01.bicos.io", false);
		//vSeeds.emplace_back("mainnet02.bicos.io", false);
		//vSeeds.emplace_back("mainnet03.bicos.io", false);
        DNS_SEED_MAINNET;
...
public:
    CTestNetParams() {
        ...
        //consensus.BCSHeight = std::numeric_limits<int>::max();//1;
        consensus.BCSZawyLWMAHeight = -1; // Activated on testnet
        consensus.BCSEquihashForkHeight = std::numeric_limits<int>::max(); // Not activated on testnet
        //consensus.BCSPremineWindow = 50;
        //consensus.BCSPremineEnforceWhitelist = false;
        ...
        consensus.nZawyLwmaAveragingWindow = 45;
        consensus.nZawyLwmaAjustedWeight = 13632;
        consensus.BCSMaxFutureBlockTime = 7 * 10 * 60; // 70 mins
		...
		//vSeeds.emplace_back("testnet01.bicos.io", false);
		//vSeeds.emplace_back("testnet02.bicos.io", false);
		//vSeeds.emplace_back("testnet03.bicos.io", false);
        DNS_SEED_TESTNET;
...
public:
    CRegTestParams() {
        ...
        //consensus.BCSHeight = std::numeric_limits<int>::max(); //2000;
        consensus.BCSZawyLWMAHeight = -1; // Activated on regtest
        consensus.BCSEquihashForkHeight = 2001;
        //consensus.BCSPremineWindow = 10;
        //consensus.BCSPremineEnforceWhitelist = false;
        consensus.powLimit = uint256S("ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
        consensus.powLimitStart = uint256S("ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
        consensus.powLimitLegacy = uint256S("ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
...
//class BICOScoinAddressChainParam : public CMainParams
//{
//public:
//    BICOScoinAddressChainParam()
//    {
...
//static BICOScoinAddressChainParam chainParamsForAddressConversion;
...
bool CChainParams::IsPremineAddressScript(const CScript& scriptPubKey, uint32_t height) const {
    ...
    assert(0/*(uint32_t)consensus.BCSHeight <= height*/ &&
           1/*height < (uint32_t)(consensus.BCSHeight + consensus.BCSPremineWindow)*/);
    //int block = height - consensus.BCSHeight;
    //int num_unlocked = consensus.BCSPremineWindow * 40 / 100;  // 40% unlocked.
    //int num_locked = consensus.BCSPremineWindow - num_unlocked;  // 60% time-locked.
    ...
    //    int lock_time = consensus.BCSHeight + stage_lock_time * (1 + stage);
