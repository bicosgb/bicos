...
#include "bcs_cltv_multisig_data.h"
#include "test_bicoscoin.h"

BOOST_FIXTURE_TEST_SUITE(bcs_tests, BasicTestingSetup)
...
BOOST_AUTO_TEST_CASE(zcash_header_compatible)
{
    ...
    bcs_header.nHeight = 10000000;
    // `CBlockHeader.nReserved[5:7]` should be placed in the last 8 bytes of `ZcashBlockHeader.hashReserved`.
    bcs_header.nReserved[5] = 1234;
    bcs_header.nReserved[6] = 0;
    bcs_header.nSolution = std::vector<unsigned char> {
        0x11, 0x22, 0x33
    };
    ...
    BOOST_CHECK_EQUAL(bcs_header.nHeight, (uint32_t)zcash_header.hashReserved.GetUint64(0));
    BOOST_CHECK_EQUAL(bcs_header.nReserved[5], (uint32_t)zcash_header.hashReserved.GetUint64(3));
    BOOST_CHECK_EQUAL(bcs_header.nSolution[2], zcash_header.nSolution[2]);
}
...
std::string ConvertToNewAddress(const std::string& old_address)
{
    ...
    CBICOScoinAddress new_addr(addr.Get(fakeLegacyChainParam));
    return new_addr.ToString();
}

std::string ConvertToOldAddress(const std::string& new_address)
{
    CBICOScoinAddress addr(new_address);
    BOOST_CHECK(addr.IsValid());
    CBICOScoinAddress old_addr(addr.Get(), fakeLegacyChainParam);
