...
*Setup instructions for a Gitian build of BICOScoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BICOScoin
...
- [Building BICOScoin Core](#building-bicoscoin-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-bicoscoin-host/README.md).
...
Clone the git repositories for BICOSCOIN and Gitian.
...
in the bicoscoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building BICOScoin Core
----------------

To build BICOScoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the bicoscoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/BICOSCOIN/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/bicosgb/bicos
...
For example:
```bash
URL=https://github.com/akx20000a/BICOSCOIN.git
COMMIT=d5522ed66a924a2d505fe45ed2487e741c6c0917
./bin/gbuild --commit BICOSCOIN=${COMMIT} --url BICOSCOIN=${URL} ../BICOSCOIN/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit BICOSCOIN=${COMMIT} --url BICOSCOIN=${URL} ../BICOSCOIN/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit BICOSCOIN=${COMMIT} --url BICOSCOIN=${URL} ../BICOSCOIN/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the bicoscoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bicoscoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/BICOSCOIN
SIGPATH=/some/root/path/bicoscoincore-detached-sigs

./bin/gbuild --url BICOSCOIN=${BTCPATH},signature=${SIGPATH} ../BICOSCOIN/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bicoscoin-core-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bicoscoin-core-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bicoscoin-core-osx-build.assert
```
...
[BICOSCOIN/gitian.sigs](https://github.com/bicos/gitian.sigs/) repository, or if that's not possible create a pull
request.
