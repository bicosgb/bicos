...
# Copyright (c) 2010-2016 The BICOScoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
"""BICOScoin P2P network half-a-node.

This python code was modified from ArtForz' public domain  half-a-node, as
found in the mini-node branch of http://github.com/jgarzik/pynode.

NodeConn: an object which manages p2p connectivity to a bicoscoin node
NodeConnCB: a base class that describes the interface for receiving
            callbacks with network messages from a NodeConn
CBlock, CTransaction, CBlockHeader, CTxIn, CTxOut, etc....:
    data structures that should map to corresponding structures in
    bicoscoin/primitives
...
COIN = 100000000 # 1 BCS in satoshis
...
BCS_REGTEST_HARDFORK_HEIGHT = 2000
...
# Objects that map to bicoscoind objects, which can be serialized/deserialized
...
class CTransaction(object):
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in bicoscoind
            if (flags != 0):
                self.vin = deser_vector(f, CTxIn)
...
class CBlockHeader(object):
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            if self.nHeight < BCS_REGTEST_HARDFORK_HEIGHT:
...
class msg_headers(object):
    ...
    def deserialize(self, f):
        # comment in bicoscoind indicates these should be deserialized as blocks
...
class NodeConnCB(object):
    """Callback and helper functions for P2P connection to a bicoscoind node.
...
class NodeConn(asyncore.dispatcher):
    ...
    def __init__(self, dstaddr, dstport, rpc, callback, net="regtest", services=NODE_NETWORK, send_version=True):
        ...
        logger.info('Connecting to BICOScoin Node: %s:%d' % (self.dstaddr, self.dstport))
