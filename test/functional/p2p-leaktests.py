...
Also test that nodes that send unsupported service bits to bicoscoind are disconnected
...
class CNodeNoVersionBan(CLazyNode):
    ...
    # NOTE: implementation-specific check here. Remove if bicoscoind ban behavior changes
    def on_open(self, conn):
        super().on_open(conn)
...
class P2PLeakTest(BICOScoinTestFramework):
    def __init__(self):
        super().__init__()
        self.num_nodes = 1
